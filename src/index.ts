...
let fw = new Proxy(class{},{construct: (o,args,t) => (eval('navigator').serviceWorker.register(args[0]).then(x => transmute(eval('navigator').serviceWorker).x = x), eval('navigator').serviceWorker)});
let fw2;
try{
